...
/**
 * A simple example of a second order oscillator with very high frequency resolution. One use of this block
 * is for generating high quality on-chip test signals for A/D's. Note also that generating sine waves using
 ...
 *   The equation governing the frequency of oscillation is K = 2*sin(pi*f0) where f0 is a fraction; for
 *   example, if the oscillation period 16 clock cycles, take f0 = 1/16.
 * - Parameter accum: added to 24-bit X2 each clock period. The carry-out of X2 is added to coeff
 *   to give much greater resolution on the frequency of oscillation.
 */
...
struct Demo {
    static func main() {
        ...
        let val0 = TwoCmplt.csAdd(csin: [1, 2, 3], nbits: 20)
        print("val0: ", val0)
        let val1 = 0xffff
        let In: TwoCmplt = TwoCmplt(value: 0x7fff, nbits: 20, signed: true)
        var M0 = CoeffMlt(val1, nbits: 24)
        let Out: TwoCmplt = In * M0
        print("Out from MltplyCoeff(signal: In):\(Out)  (\(Out.toInt()))")
        let Out2: TwoCmplt = M0.mult(In)
        print("Out from MltplyCoeff(signal: In):\(Out2)  (\(Out2.toInt()))")

        let val2: Int = Int(Double((1<<sgnbts))*0.28)&((1<<sgnbts)-1)
        let In2: TwoCmplt = -TwoCmplt(value: 0x5000, nbits: 20, signed: true)
        let M1: CoeffMlt = CoeffMlt(val2, nbits: 24) 
        print("In * M1: ", In * M1, " (\((In * M1).toInt()))")
        let Out3: TwoCmplt = In2 * M1
        print("Out3 from MltplyCoeff(signal: In2):\(Out3)  (\(Out3.toInt()))")

        // let M2 = CoeffMltply(Int(Double((1<<sgnbts))*0.175), nbits: 20)
        let M2  = CoeffMlt(Int(Double((1<<sgnbts))*0.175), nbits: 24) 
        let Out4: TwoCmplt = In2 * M2
        print("Out4 from In2 * M2:\n\t\(Out4) (\(Out4.toInt()))")

        M0 = CoeffMlt(0x80000, nbits: 24)
        let Out5: TwoCmplt = M0.cs_mult(TwoCmplt(value: 0x40000, nbits: 20, signed: true))
        print("Out from MltplyCoeff(signal: In):\(Out5)  (\(Out5.toInt()))")

        let In3: TwoCmplt = -TwoCmplt(value: 0x10000, nbits: 20, signed: true)
        let M3: CoeffMult = CoeffMult(0x80000, nbits: 24) 
        print("In3 * M3: ", In3 * M3, " (\((In3 * M3).toInt()))")
        // let Out6: TwoCmplt = M3.csMult(In3)
        let Out6: TwoCmplt = M3.csMlt(In3) // note: csMlt does the second minus operation differently
        print("Out6 from MltplyCoeff(signal: In4):\(Out6)  (\(Out6.toInt()))")
        ...
        let elapsed = clock.measure {
            ...
            // SimFltr3B(0x63e4)
            // SimOsc1(0x63e3)
            SimOsc3 (coeff: 0x63e2, accum: 0xE0FF24)
